 ...
 *    - 특벽한 목적의 작업을 수행하기 위해 독립적으로 설계된 코드
 *    - 기능을 수행하는 코드 단위
 *    - 반복적인 작업을 피하기 위해 사용
 ...
 * 
 *  함수의 종류
 *  1. 내장함수(built-in): C언어 설치 내장 함수 ex) main scanf pritnf
 *  2. 외장함수(library) : 다른 개발자가 만들어 놓은 함수
 *  3. 사용자 정의 함수   : 직접 만듬                                                다 많이씀
 * 
 *  규칙 
...
// 1) 함수 정의: 함수 코드 구현          //sum 함수에서 sun함수 호출 //int(형) return(값) 같이씀 
